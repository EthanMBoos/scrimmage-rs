...
fn excluded_straight_spawning_and_camera_options_are_errors() {
    for key in [
        ...
        let error = Straight::configure(&PluginParams::text(&params))
            .err()
...
fn unported_controller_and_motion_modes_are_errors() {
    for key in ["use_roll", "use_glide_slope"] {
        let params = Params::from([(key.into(), "true".into())]);
        assert!(SimpleAircraftControllerPid::configure(&PluginParams::text(&params)).is_err());
    }
    let params = Params::from([("override_heading".into(), "true".into())]);
    assert!(SingleIntegrator::configure(&PluginParams::text(&params)).is_err());
}
...
fn boundary_and_ground_reject_unimplemented_geometry_and_response() {
    for (key, value) in [
        ...
        assert!(
            Boundary::configure(&PluginParams::text(&params)).is_err(),
            "{key}"
        );
    }
    for (key, value) in [
        ...
        let params = Params::from([(key.into(), value.into())]);
        assert!(
            GroundCollision::configure(&PluginParams::text(&params)).is_err(),
            "{key}"
        );
    }
}
...
fn routes_require_finite_points_positive_motion_and_a_complete_update() {
    for (key, value) in [
        ...
        let params = Params::from([(key.into(), value.into())]);
        assert!(
            WaypointFollower::configure(&PluginParams::text(&params)).is_err(),
            "{key}={value}"
        );
    }
    for (key, value) in [("update_at_s", "1"), ("update_waypoints", "1,2,3")] {
        let params = Params::from([(key.into(), value.into())]);
        assert!(WaypointBroadcast::configure(&PluginParams::text(&params)).is_err());
    }
}
