...
use crate::math::Vec3;
use crate::scenario::{
    EndConditions, EntityGroupConfig, PluginConfig, PluginValues, RunConfig, ScenarioConfig, Spawn,
};
...
pub(super) fn load(
    ...
    Ok(Mission {
        ...
        viewer: boolean(run, "enable_gui", false)?,
    ...
    if let (Some(rate_text), Some(_)) = (params.get("generate_rate"), params.get("generate_count"))
    {
        ...
        if rate_hz > 0.0 && batch_count > 0 {
            spawn = Some(Spawn {
                ...
                // C++ calls this variance but uses it as a standard deviation.
                time_stddev_s: number(params, "generate_time_variance", 0.0)?,
            });
        }
    }
    let mut velocity_mps = Vec3::new(
        number(params, "vx", 0.0)?,
        number(params, "vy", 0.0)?,
        number(params, "vz", 0.0)?,
    );
    let speed_mps = number(params, "speed", 0.0)?;
    // C++ applies scalar speed along world X only when velocity is zero.
    if speed_mps > 0.0 && velocity_mps == Vec3::zeros() {
        velocity_mps = Vec3::new(speed_mps, 0.0, 0.0);
    }
    Ok(EntityGroupConfig {
        label,
        team: params
            .get("team_id")
...
mod tests {
    ...
    #[test]
    fn scalar_speed_translates_to_world_velocity_only_when_velocity_is_zero() -> Result<()> {
        let mut params = Params::from([
            ("speed".into(), "12".into()),
            ("heading".into(), "90".into()),
        ]);
        let read = |params: &Params| {
            entity(
                "test".into(),
                params,
                Vec::new(),
                Vec::new(),
                PluginConfig::new("SingleIntegrator"),
                Vec::new(),
            )
        };
        assert_eq!(read(&params)?.velocity_mps, Vec3::new(12.0, 0.0, 0.0));
        params.insert("vy".into(), "3".into());
        assert_eq!(read(&params)?.velocity_mps, Vec3::new(0.0, 3.0, 0.0));
        Ok(())
    }
    ...
    fn mission_values_and_param_common_reach_the_plugin() -> Result<()> {
        ...
        let defaults = &mission.scenario.entities[0];
        assert!(text(&defaults.autonomy[0]).is_empty());
        assert!(text(&defaults.motion_model).is_empty());

        let overrides = &mission.scenario.entities[1];
        assert_eq!(text(&overrides.autonomy[0])["speed"], "26");
        assert_eq!(text(&overrides.motion_model)["turning_radius"], "23");
        assert_eq!(text(&overrides.motion_model)["min_velocity"], "16");
        assert!(!text(&overrides.motion_model).contains_key("param_common"));
        assert_eq!(text(&overrides.sensors[0])["stddev_m"], "0");
        mission
            .scenario
            .resolve(&crate::plugin::PluginRegistry::with_builtins())?;
        Ok(())
    }
    ...
    fn misspelled_plugin_parameters_are_errors() -> Result<()> {
        let root = root();
        let mut mission = Mission::load(
            &root.join("crates/core/tests/fixtures/plugin_defaults.xml"),
            &root,
            &Params::new(),
        )?;
        let PluginValues::Text(params) = &mut mission.scenario.entities[1].motion_model.params
        else {
            panic!("XML plugins hold text");
        };
