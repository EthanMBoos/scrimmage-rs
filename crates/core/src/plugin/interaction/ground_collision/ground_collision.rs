...
impl Plugin for GroundCollision {
    ...
    fn configure(params: &PluginParams<'_>) -> Result<GroundCollisionConfig> {
        ...
        Ok(GroundCollisionConfig {
            height_world_m: params.number("ground_collision_z", 0.0)?,
            startup_collisions: params.boolean("enable_startup_collisions", true)?,
            team_id,
        })
    }
